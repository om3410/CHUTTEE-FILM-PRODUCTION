...
from apps.production.models import (
  BudgetTransaction, ProductionRisk, FestivalSubmission, CrewMember, CastMember, FilmProject, ShootDay,
)

class BudgetByCategoryAPIView(APIView):
  permission_classes = [IsAuthenticated]
  def get(self, request):
    data = (BudgetTransaction.objects.values('category')).annotate(total=Sum('amount').order_by('-total'))
...
class DailyBurnRateAPIView(APIView):
  ...
  def get(self, request):
    ...
    for t in BudgetTransaction.objects.all().order_by('transaction_date'):
      cumulative += float(t.amount or 0)
      result.append({
        "date": t.transition_date,
        "daily_spend": float(t.amount or 0),
        "cumulative_spend": cumulative,
      })
    return Response(result)

class BudgetSummarAPIView(APIView):
  permission_classes = [IsAuthenticated]
  def get(self, request):
...
class RiskMatrixAPIView(APIView):
  ...
  def get(self, request):
    ...
    return Response([{
      ...
      "severity": r.severity,
      "probabillity": float(r.probability or 0),
...
class CrewAvailabilityAPIview(APIView):
  ...
  def get(self, request):
    ...
    availble = CrewMember.objects.exclude(id__in=busy_ids)
    return Response(list(availble.values('id', 'full_name', 'role')))

class UpcomingFestivalAPIView(APIView):
  permission_classes = [IsAuthenticated]
  def get(self, request):
